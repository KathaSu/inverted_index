...
class Index():
    ...
    def preprocessing(self, document):
        # Assign the Snowball Stemmer to the "stemmer" variable. The stemmer reduces morpholigical endings.
        stemmer = PorterStemmer()
        # Using regular expressions to exclude anything different from words and whitespaces in the document.
    ...
    def index(self, documents):
        ...
        # Read data using pandas. Use separation of tabs and assign it to the variable "documents".
        ...
        for document_id, document_text in zip(documents.id, documents.news_text):
            ...
            for token in tokens:
                ...
                if token not in dictonary:
                    # Add token to the dictionary with the information that the length 
                    # of the postings list is 1 and the current pointerlist_id.
                    dictonary.update({token: [1, pointerlist_id]})
                    # Add entry to postings_list that contains the pointerlist id and the document id. 
                    # Through the pointerlist id we can connect the dictionary with the postings list.
